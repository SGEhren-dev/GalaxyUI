...
void glxyui::begin_groupbox(std::string_view title, const vec2 size, const int flags) {
    ...
    if (flags & glxyui_groupbox_flags_legacy_design) {
    ...
    utils::misc::push_cursor_pos(vec2{ cursor_pos.x + 8, cursor_pos.y + ((flags & glxyui_groupbox_flags_legacy_design) ? 0 : header_height) + 8 });

    utils::misc::push_font(font);
}

// ========================================================================
void glxyui::end_groupbox() {
