...
bool glxyui::clickable_text(const char *id) {
    ...
    window_ctx.render.emplace_back(glxyui_control_render_t{{draw_pos.x, draw_pos.y}, glxyui_render_type::glxyui_text, (hovered || window_ctx.blocking == utils::hash::hash(id)) ? global_colors.control_active_or_clicked : global_colors.color_text, id_split[0], vec2{0, 0}, font});

    utils::misc::push_cursor_pos(vec2{cursor_pos.x + text_width + global_config.item_space, cursor_pos.y});
    utils::misc::push_cursor_pos(vec2{cursor_pos.x, cursor_pos.y + text_height / 2 + global_config.item_space});
...
// ========================================================================
void glxyui::text(const char *text) {
    const unsigned long font = utils::misc::pop_font();
    ...
    window_ctx.render.emplace_back(glxyui_control_render_t{{draw_pos.x, draw_pos.y}, glxyui_render_type::glxyui_text, global_colors.color_text, text, vec2{0, 0}, font});
