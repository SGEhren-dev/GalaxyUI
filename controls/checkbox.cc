...
void glxyui::checkbox(const char* id, bool& value) {
    ...
    window_ctx.render.emplace_back(glxyui_control_render_t{ { draw_pos.x + 14, draw_pos.y - 2 }, glxyui_render_type::glxyui_text,
        value ? global_colors.color_text : global_colors.color_text_dimmer, id_split[0], vec2{ 0, 0 }, font });
    window_ctx.render.emplace_back(glxyui_control_render_t{ { draw_pos.x + 1, draw_pos.y + 1 }, glxyui_render_type::glxyui_filled_rect,
        value ? global_colors.control_active_or_clicked : global_colors.control_idle, "", { control_width - 2, control_height - 2 } });
    window_ctx.render.emplace_back(glxyui_control_render_t{ { draw_pos.x, draw_pos.y }, glxyui_render_type::glxyui_filled_rect,
         global_colors.control_outline, "", { control_width, control_height } });

    utils::misc::push_cursor_pos(vec2{ cursor_pos.x + 14 + text_width + global_config.item_space, cursor_pos.y });
