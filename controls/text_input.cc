...
void glxyui::text_input(const char *id, std::string &value, const int max_length, const int flags) {
    ...
    window_ctx.render.emplace_back(glxyui_control_render_t{{draw_pos.x + 4, draw_pos.y + 4}, glxyui_render_type::glxyui_text, global_colors.color_text, flags & glxyui_text_input_flags_password ? std::string(value.length(), '*').c_str() : value.c_str(), vec2{0, 0}, font});
    window_ctx.render.emplace_back(glxyui_control_render_t{{draw_pos.x + 1, draw_pos.y + 1}, glxyui_render_type::glxyui_filled_rect, active ? global_colors.control_active_or_clicked : global_colors.control_idle, "", {control_width - 2, control_height - 2}});
